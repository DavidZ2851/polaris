...
def main(eval_args: EvalArgs):
    ...
    render_gripper_only = (
        eval_args.policy.render_gripper_only
        or eval_args.policy.client == "YOLH"
    )
    env: ManagerBasedRLSplatEnv = gym.make(  # type: ignore
        eval_args.environment,
        cfg=env_cfg,
        render_gripper_only=render_gripper_only,
    )
