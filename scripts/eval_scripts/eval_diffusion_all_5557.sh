#!/bin/bash
# Evaluate checkpoints sequentially on port 5557.
# Server on GPU 1, Eval on GPU 0.
# Run in one terminal while eval_diffusion_all_5556.sh runs in another.
#
# Checkpoints: human100_robot5, human40_robot40, human5_robot5, robot5
...
CKPTS=(
    "$CKPT_DIR/human100_robot5.ckpt"
    "$CKPT_DIR/human40_robot40.ckpt"
    "$CKPT_DIR/human5_robot5.ckpt"
    "$CKPT_DIR/robot5.ckpt"
)

echo "Checkpoints (port $PORT, server GPU$SERVER_GPU, eval GPU$EVAL_GPU — both evals on GPU1):"
