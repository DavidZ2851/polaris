...
class ManagerBasedRLSplatEnv(ManagerBasedRLEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # If enabled, only the Robotiq 2F-85 splats are rendered for the robot.
        self.render_gripper_only = render_gripper_only
    ...
    @staticmethod
    def _is_gripper_splat_name(name: str) -> bool:
        return name.startswith("Gripper-Robotiq_2F_85")
        ...
        if expensive:
            ...
            for cam in mask_and_rgb:
                ...
                if "wrist" in cam:
                    ...
                    if self.render_gripper_only:
                        # In gripper-only mode, keep pure splat rendering for non-wrist cameras.
                        rgb[cam] = og_img
                    else:
                        new_img = np.where(mask, sim_img, og_img)
                        rgb[cam] = new_img
                rgb[f"{cam}_mask"] = mask
            ...
            for cam in self.scene.sensors:
                if isinstance(self.scene.sensors[cam], Camera):
                    mask = (
                        self.scene[cam]
                        .data.output["semantic_segmentation"][0]
                        .detach()
                        .cpu()
                        .numpy()
                    )
                    ...
                    rgb[f"{cam}_mask"] = np.where(mask >= 2, 1, 0)
    ...
    def setup_splat_robot(self):
        ...
        for ply in sorted(list(robot_asset_path.glob("SEGMENTED/*.ply"))):
            if self.render_gripper_only and not self._is_gripper_splat_name(ply.stem):
                continue
